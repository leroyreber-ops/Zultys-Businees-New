      ...
      {isVisible && (
        <>
          ...
          <motion.div 
            ...
            transition={{ type: 'spring', damping: 25, stiffness: 150, delay: 0.5 }}
            className="fixed bottom-0 left-0 right-0 z-50 md:hidden bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200/80 dark:border-slate-800/80 px-4 py-3 shadow-[0_-8px_30px_rgba(0,0,0,0.12)] pb-[calc(12px+env(safe-area-inset-bottom))]"
            id="mobile-sticky-cta-bar"
          >
            <div className="flex gap-3 max-w-md mx-auto items-center">
              <a
                href={`sms:817-231-2962?body=${encodeURIComponent("Hello Zulty's Team, I'm searching information on the Best Zulty's business phone solution for my company as soon as possible please contact me with your available options!")}`}
                className="flex-1 flex items-center justify-center gap-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-750 text-slate-800 dark:text-slate-200 font-extrabold py-3 px-4 rounded-xl border border-slate-250 dark:border-slate-700 text-sm transition-all active:scale-95 text-center"
                id="mobile-cta-sms-btn"
              >
              ...
              <a
                ...
                className="flex-[1.5] flex items-center justify-center gap-2 bg-gradient-to-r from-zultys-green to-emerald-600 text-white font-extrabold py-3 px-4 rounded-xl shadow-[0_4px_14px_rgba(0,168,45,0.35)] text-sm transition-all active:scale-95 text-center"
                id="mobile-cta-call-btn"
              >
