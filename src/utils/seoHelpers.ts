...
function updateSchema(props: SEOProps) {
  ...
  if (props.productSchema) {
    const hasValidPrice = Boolean(
      props.productSchema.price &&
      !['0', '0.00', 'Contact for Quote', 'Call for price', 'Request quote', 'TBD'].includes(String(props.productSchema.price).trim())
    );

    const productSchema = {
      "@context": "https://schema.org",
      "@type": "Product",
      "@id": `${props.canonicalUrl}#product`,
      ...
      ...(hasValidPrice && {
        "offers": {
          ...
          "price": props.productSchema.price,
          ...(props.productSchema.availability && { "availability": props.productSchema.availability }),
          "seller": {
            "@type": "Organization",
...
export function applyProductSEO(name: string, description: string, url: string, keywords: string, extra?: any) {
  applySEO({
    ...
    productSchema: {
      ...
      model: extra?.model,
      price: extra?.price,
      priceCurrency: extra?.price ? (extra?.priceCurrency || 'USD') : undefined,
      availability: extra?.price ? extra?.availability : undefined,
      sku: extra?.sku
    }
  });
}
